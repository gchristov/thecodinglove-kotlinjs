...
class SlackInteractivityPubSubHandler(
...
) : BasePubSubHandler<PubSubSlackInteractivityMessage>(
    ...
    override fun deserialisationStrategy() = PubSubSlackInteractivityMessage.serializer()

    override suspend fun handlePubSubRequest(body: PubSubSlackInteractivityMessage): Either<Throwable, Unit> = either {
        when (body.payload) {
            is PubSubSlackInteractivityMessage.InteractivityPayload.InteractiveMessage -> body.payload.handle()
        }.mapLeft {
    ...
    private suspend fun PubSubSlackInteractivityMessage.InteractivityPayload.InteractiveMessage.handle(): Either<Throwable, Unit> =
        either {
            ...
            if (selfDestruct5MinAction != null) {
                analytics.sendEvent(
                    clientId = user.id,
